...
/**
 * Created by lhfei on 2017/8/4.
 */

/**
 * 自定义View，圆形进度条
 */
public class CircleProgressButton extends ProgressBar {
    ...
    private static final int PROGRESS_CENTER_COLOR = 0xFF01A1EB;//进度条中心颜色  （绘制播放、暂停、等待的地方）
    private static final int PROGRESS_REACHED_WIDTH = 2;//进度条的宽度
    private static final int PROGRESS_DEFAULT_WIDTH = 2;//默认圆的宽度
    ...
    //默认圆(边框)的颜色
    private int mCenterColor = PROGRESS_CENTER_COLOR;
    //进度条的高度
    ...
    public CircleProgressButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性的值
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.CircleProgressButton);
        //默认圆的颜色
        mDefaultColor = array.getColor(R.styleable.CircleProgressButton_progress_default_color, PROGRESS_DEFAULT_COLOR);
        //进度条的颜色
        mReachedColor = array.getColor(R.styleable.CircleProgressButton_progress_reached_color, PROGRESS_REACHED_COLOR);
        //进度条的颜色
        mCenterColor = array.getColor(R.styleable.CircleProgressButton_progress_center_color, PROGRESS_CENTER_COLOR);
        //默认圆的高度
        mDefaultWidth = (int) array.getDimension(R.styleable.CircleProgressButton_progress_default_width, mDefaultWidth);
        //进度条的高度
        mReachedWidth = (int) array.getDimension(R.styleable.CircleProgressButton_progress_reached_width, mReachedWidth);
        //圆的半径
        mRadius = (int) array.getDimension(R.styleable.CircleProgressButton_circle_radius, mRadius);
        //最后不要忘了回收 TypedArray
        array.recycle();
...
//        int heightSize = MeasureSpec.getSize(heightMeasureSpec);
        ...
        int paintHeight = Math.max(mReachedWidth, mDefaultWidth);//比较两数，取最大值
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(mDefaultWidth);
        ...
        canvas.drawArc(mRectF, -90, sweepAngle, false, mPaint);//drawArc：绘制圆弧
        ...
        switch (mStatus) {
            case WAIT:
                ...
                canvas.drawLine(mRadius, mRadius / 3, mRadius, mRadius, mPaint);
                canvas.drawLine(mRadius, mRadius, mRadius * 5 / 3, mRadius, mPaint);
                break;
            case RUN://正在进行状态,画两条竖线
                mPaint.setStyle(Paint.Style.STROKE);
                mPaint.setStrokeWidth(dp2px(5));
                mPaint.setColor(mCenterColor);
            ...
            case ERROR:
                ...
                canvas.drawLine(mRadius * 2 / 3, mRadius * 2 / 3, mRadius * 4 / 3, mRadius * 4 / 3, mPaint);//右斜线
                canvas.drawLine(mRadius * 4 / 3, mRadius * 2 / 3, mRadius * 2 / 3, mRadius * 4 / 3, mPaint);
                break;
            case FINISH: //有了path之后就可以在onDraw中绘制三角形的End和Starting状态了
                mPaint.setStyle(Paint.Style.STROKE);
                mPaint.setStrokeWidth(dp2px(5));
                mPaint.setColor(Color.parseColor("#ff99cc00"));
                mPaint.setStrokeCap(Paint.Cap.ROUND);
                canvas.drawLine(mRadius * 2 / 3, mRadius, mRadius, mRadius * 4 / 3, mPaint);//勾短边
                canvas.drawLine(mRadius, mRadius * 4 / 3, mRadius * 3 / 2, mRadius * 2 / 3, mPaint);
                break;
        }
        canvas.restore();
    }
